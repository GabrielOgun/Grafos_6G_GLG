...
TGrafoND::TGrafoND(int n){
  ...
  // Aloca o array classes
  this->classes = new int[n];
  for (int i = 0; i < n; i++) {
      classes[i] = -1;  // Inicializa todas as classes como -1
  }
...
void TGrafoND::setPorcentagemDoenca(float novaPorcentagem){
    for(int i = 0; i < n; i++){
        for(int j = i+1; j < n; j++ ){
            adj[i][j] = adj[i][j] / porcentagemDoenca;
            adj[i][j] = adj[i][j] * novaPorcentagem;
        }
    }
...
int TGrafoND::podeColorir(int v, int c) {
    for (int i = 0; i < n; i++) {
        if (adj[v][i] != INF && classes[i] == c) {
            return 0;
        }
    }
    return 1;
...
// Adicione o seguinte método público à classe TGrafoND
void TGrafoND::coloracaoPorClasse() {
    int k = 0; // Número da classe de cores

    for (int i = 0; i < n; i++) {
        if (classes[i] == -1) {
            classes[i] = k;
            for (int j = 0; j < n; j++) {
                if (i != j && classes[j] == -1 && podeColorir(j, k)) {
                    classes[j] = k;
                }
            }
            k++;
    ...
    // Imprimir a coloração
    for (int i = 0; i < k; i++) {
        cout << "Classe de cor " << i+1 << ": ";
        for (int j = 0; j < n; j++) {
            if (classes[j] == i) {
                cout << j << " ";
            }
...
float TGrafoND::max(int a, int b)
{
    return (a > b) ? a : b;
}

void TGrafoND::dfs(int visitados[], int u, int destino, int produto)
{
    if (u == destino) // Se chegamos ao destino, atualizamos o maior produto
    {
        maxProduto = max(maxProduto, produto);
        return;
    }

    visitados[u] = 1; // Marca o vértice atual como visitado

    // Percorre todos os vértices adjacentes não visitados
    for (int v = 0; v < n; v++)
    {
        if (!visitados[v] && adj[u][v] != 0)
        {
            // Faz a chamada recursiva para o próximo vértice
            dfs(visitados, v, destino, produto * adj[u][v]);
        }
    }

    visitados[u] = 0; // Marca o vértice atual como não visitado para que outros caminhos possam usá-lo
}

float TGrafoND::encontrarMaiorProduto(int origem, int destino)
{
    int visitados[n]; // Array para rastrear os vértices visitados

    // Inicializa o array de visitados como falso
    for (int i = 0; i < n; i++)
    {
        visitados[i] = 0;
    }

    // Realiza a busca em profundidade para encontrar o maior produto
    dfs(visitados, origem, destino, 1);
